  ...
  return (
    <>
      <div className="topbar">
        <div>
          <div className="eyebrow">{live.length ? "Live impact intelligence" : "Impact intelligence"}</div>
          <h1>Good morning, impact team.</h1>
          <p className="muted">
            {live.length
              ? "Connected to your Cloudinary evidence workspace."
              : "Demo workspace — connect Cloudinary for live metrics."}
          </p>
        </div>
      ...
      <div className="grid4">
        {stats.map(([label, value, detail, Icon]) => (
          <div className="card stat" key={label}>
            <Icon size={18} color="#63e6a3" />
            <div className="statValue">{value}</div>
            <div className="muted">{label}</div>
            <div className="statDelta">{detail}</div>
          </div>
        ))}
      </div>
      ...
      <section className="section">
        <div className="sectionHead">
          ...
          <Link className="btn" href="/media">
            View library <ArrowUpRight size={14} style={{ verticalAlign: "middle" }} />
          </Link>
        </div>
        ...
        <div className="mediaGrid">
          {items.slice(0, 3).map((m) => (
            <Link
              href={live.length ? `/media/view?id=${encodeURIComponent(m.asset_id)}` : "/media"}
            ...
            >
              ...
              <div className="mediaInfo">
                ...
                <p className="muted">{m.location} · {m.date}</p>
