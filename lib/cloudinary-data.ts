...
function normalize(r: CloudinaryResource): ImpactMedia {
  ...
  const date = r.created_at ? new Date(r.created_at).toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"}) : "Unknown date";
...
async function admin(path: string, init?: RequestInit) {
  ...
  if (!response.ok) throw new Error(data?.error?.message || "Cloudinary Admin API request failed");
  return data;
}
...
export async function getCloudinaryMedia(limit=100): Promise<ImpactMedia[]> {
  ...
  const params = new URLSearchParams({
    ...
    tags: "true",
    context: "true",
    fields: "public_id,asset_id,resource_type,type,format,created_at,width,height,bytes,secure_url,url,tags,context,asset_folder,display_name",
  });
...
export async function getCloudinaryAsset(assetId: string): Promise<ImpactMedia|null> {
  ...
  const data = await admin(`/resources/by_asset_ids?asset_ids[]=${encodeURIComponent(assetId)}&tags=true&context=true&metadata=true`);
...
export async function visualSearchCloudinaryMedia(query: string): Promise<ImpactMedia[]> {\n  if (!cloudinaryConfigured() || !query.trim()) return [];\n  try {\n    const data = await admin(`/resources/visual_search?text=${encodeURIComponent(query)}&max_results=30`);\n    return (data?.resources || []).map(normalize);\n  } catch { return []; }\n}\n\nexport async function searchCloudinaryMedia(query: string): Promise<ImpactMedia[]> {
  ...
  return all
    .map(item => {
      const haystack = [
        item.title,item.project,item.location,item.activity,
        ...(item.tags || []), ...Object.values(item.metadata || {})
      ].join(" ").toLowerCase();
    ...
    .map(x=>({...x.item,score:Math.min(99,Math.max(60,Math.round((x.hits/terms.length)*100))) }))
    .slice(0,30);
}

export async function updateCloudinaryContext(assetId: string, context: Record<string,string>) {
  if (!cloudinaryConfigured()) return null;
  const data = new URLSearchParams();
  for (const [key,value] of Object.entries(context)) data.append(`context[${key}]`, value);
